...
const buildLet = (context, [_let, symbol, binding, body]) =>
  // TODO: don't make arrow async if body contains no awaits
  appears(symbol, body)
    ? Call(Arrow([SafeId(nameOf(symbol))], uncast(build(context.add([asSymbol(symbol), {}]), body)), context.async),
...
module.exports = (options = {}) => {
  ...
  const context = new Context({ async: options.async, head: true, locals: new Map(), inlines, awaitedInlines });
  ...
  const $ = {
    ...
    asJsBool, asShenBool, asNzNumber, asNeString, symbols, inlines, awaitedInlines, functions,
