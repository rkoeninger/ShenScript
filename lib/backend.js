...
const assemble = (f, ...fabrs) => fabricate(f(...fabrs.map(x => x.ast)), Object.assign({}, ...fabrs.map(x => x.subs)));
const hoist = (context, fabr) =>
  fabr.subs.size === 0 ? fabr : fabricate(Iife(fabr.keys.map(x => Id(x)), fabr.values, fabr.ast, context.async));
...
const ann = (dataType, ast) => ast instanceof Fabr ? assemble(x => ann(dataType, x), ast) : Object.assign(ast, { dataType });
const cast = (dataType, ast) => ast instanceof Fabr ? assemble(x => cast(dataType, x), ast) : dataType !== ast.dataType ? Object.assign(Call$('as' + dataType, [ast]), { dataType }) : ast;
const uncast = ast => ast instanceof Fabr ? assemble(x => uncast(x), ast) : ast.dataType === 'JsBool' ? cast('ShenBool', ast) : ast;
const isForm = (expr, length, lead) => isNeArray(expr) && expr[0] === symbolOf(lead) && (!length || expr.length === length);
...
module.exports = (options = {}) => {
  ...
  const inlines = { // TODO: have these return fabrs? automatically wrap asts in fabrs?
    '=': (x, y) =>
