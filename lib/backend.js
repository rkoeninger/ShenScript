const {
  ...
  Literal, Member, RawId, Return, SafeId, Sequence, Statement, Try, Unary, Vector,
  adorn, ann, generate
} = require('./ast');
...
let counter = 0;
const tempId = () => RawId(`R${counter++}$`);
const placeholder = () => {
  const id = tempId();
  return [id, x => Statement(Assign(id, x))];
};
const makeUnique = x => Id(`${nameOf(x)}${counter++}`, '$'); // TODO: weird edgecase where we want to escape and add $

const Member$ = name => Member(RawId('$'), RawId(name));
...
const idle = symbol => ann('Symbol', adorn(Member$('s'), nameOf(symbol)));
...
module.exports = (options = {}) => {
  ...
  const inlines = {
    ...
    'hd':              x => Member(cast('Cons', x), RawId('head')),
    'tl':              x => Member(cast('Cons', x), RawId('tail')),
    'error-to-string': x => ann('String', Member(cast('Error', x), RawId('message'))),
  ...
  const $ = {
    ...
    , build, context, cast, uncast, generate, tryout: expr => {
      ...
      console.log(generate(Block(...fabr.statements, Return(fabr.expression))));
    }
