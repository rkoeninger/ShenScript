...
const {
  Arrow, Assign, Async, Await, Binary, Block, Call, Catch, Class, Conditional, Const, Debugger, For,
  Generator, Id, If, Let, Literal, Member, NewObj, NewTarget, Procedure, Property, Return, SafeId,
  Sequence, Slot, Spread, Statement, Static, Super, This, Try, Unary, Var, Vector, While, Yield, YieldMany
} = require('./ast');

module.exports = $ => {
  const {
    asCons, asShenBool, asString, async, cons, consToArray, evalJs, f, fun, future, inlines,
    isArray, s, settle, symbolOf, symbols, valueOf, valueToArray, valueFromArrayTree, valueToArrayTree
  } = $;
  ...
  define('js.ast.literal',               x => Literal(x));
  define('js.ast.array',                 x => Vector(valueToArray(x)));
  define('js.ast.id',                    x => Id(x));
  define('js.ast.safe-id',               x => SafeId(x));
  define('js.ast.const',            (x, y) => Const(x, y));
  define('js.ast.let',              (x, y) => Let(x, y));
  define('js.ast.var',              (x, y) => Var(x, y));
  ...
  define('js.ast.call',          (f, args) => Call(f, args));
  define('js.ast.super',              args => Super([Spread(valueToArray(args))]));
  define('js.ast.block',              body => Block(valueToArray(body)));
  define('js.ast.seq',               exprs => Sequence(valueToArray(exprs)));
  define('js.ast.member',           (x, y) => Member(x, y));
  ...
  define('js.ast.arrow',    (params, body) => Arrow(valueToArray(params), body));
  ...
  define('js.ast.if',            (x, y, z) => If(x, y, z));
  define('js.ast.try',              (x, y) => Try(x, y));
  define('js.ast.catch',            (x, y) => Catch(x, y));
  define('js.ast.while',            (x, y) => While(x, y));
  define('js.ast.for',        (x, y, z, b) => For(x, y, z, b));
  define('js.ast.statement',             x => Statement(x));
  // TODO: template string expressions
  //       output, etc. can be overwritten to use template strings

  define('js.ast.eval',   x => evalJs(x));
  inline('js.ast.inline', x => x);
