...
module.exports = async $ => {
  const {
    asShenBool, asString, async, caller, define, defineTyped, eternal, evalJs,
    inline, isArray, nameOf, raise, s, symbol, symbolOf, toArray, toArrayTree, toList, valueOf
  } = $ = await frontend($);
  ...
  if (async) {
    ...
    await defineTyped('web.fetch*', [[s`list`, s`string`], s`-->`, [s`list`, s`string`]],
