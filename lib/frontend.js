...
module.exports = $ => {
  ...
  const defmacro = (name, fn) => { // TODO: implement defmacro
    const macrofn = expr => {
  ...
  // TODO: need a macro to convert from ({ ... }) -> (js.new-obj [...])
  // defmacro('new-obj-macro', x => {
  //   const [first, ...rest, last] = consToArray(asCons(x));
  //   if (first === symbolOf('{') && last === symbolOf('}')) {
  //     return consFromArray([symbolOf('js.new-obj'), rest]);
  //   }
  // });
  // (defmacro new-obj-macro [ { ...More... } ] -> [js.new-obj More])
