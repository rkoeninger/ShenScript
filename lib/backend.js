    ...
    for (const xs of xss.slice(1)) {
      for (const [key, value] of xs.entries) {
        ...
        if (entry) {
          ...
          //       in the case of hoisting, multiple references to the same
          //       symbol or function need to have the same value
          entry.value = value;
        } else {
...
const fabricate = (x, subs) =>
  !(x instanceof Fabr) ? new Fabr(x, subs) :
  subs                 ? new Fabr(x.ast, Object.assign({}, x.subs, subs)) :
  x;
// assemble :: ([JsExpr] -> (JsExpr | Fabr)) -> [JsExpr | Fabr] -> Fabr
const assemble = (f, ...xs) => {
...
const hoist = (context, fabr) =>
  Object.keys(fabr.subs).length === 0
    ? fabr
    : fabricate(Iife(fabr.keys.map(x => Id(x)), fabr.values, fabr.ast, context.async));
const inject = (name, f) => {
...
const idle = symbol => {
  ...
  if (name[0] === name[0].toUpperCase()) {
    // TODO: reintroduce hoisting of idle symbols starting with a captial letter
