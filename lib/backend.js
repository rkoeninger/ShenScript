...
const construct = (statements, expression) => new Construction(statements, expression);
const assemble = (combine, ...buildings) =>
  new Construction(
    flatMap(buildings, x => x.statements),
    combine(...buildings.map(x => x.expression)));
