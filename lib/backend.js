...
// TODO: for both (and) and Block, there needs to be some way to convey sibling-expression context from left to right.
//       context expires/becomes irrelevant when passing a Block boundary?
const Context = class {
  constructor(options) {
  ...
  // TODO: Context should carry a continuation that constructs expressions in terminal positions
  //       always assinging to a result variable returned ast will need result context indicating
  //       if continuation was applied or result is still an expression
  //       some continuations:
  //           continuation: x => Return(x)
  //           continuation: x => Assign(tempId, x)
  //           continuation: x => Statement(x)
...
// TODO: add b, t, u functions as members on each function for shorter calling convention
...
const makeUnique = x => SafeId(x, `${counter++}$`);
...
const complete = (context, fAst, argsAsts) =>
  context.head
    ? Call$(context.async ? 'u' : 't', [Call(fAst, argsAsts)], context.async)
...
const buildAnd = (context, [_and, left, right]) => {
  const recognised = isRecognisor(context, left);
  return assemble((lexpr, rexpr) => ann('JsBool', Binary('&&', lexpr, rexpr)),
    cast('JsBool', build(context.now(), left)),
    cast('JsBool', build(recognised ? context.ann(...recognised).now() : context.now(), right)));
};
...
const buildApp = (context, [fExpr, ...argExprs]) =>
  assemble(
    (f, ...args) => complete(context, f, args),
...
module.exports = (options = {}) => {
  ...
  const evalJs = (...args) => Func(...butLast(args).map(x => x[0]), generate(last(args)))(...butLast(args).map(x => x[1]));
  const evalFabr = fabr => evalJs(['$', $], Block(...fabr.statements, Return(fabr.expression)));
  const evalKl = expr => evalFabr(compile(valueToArrayTree(expr)));
