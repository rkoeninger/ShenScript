...
module.exports = async $ => {
  ...
  const setSignedFuncs = eternal('shen.set-signedfuncs');
  const incInfs = eternal('shen.incinfs');
  const newpv = eternal('shen.newpv');
  const unify = eternal('unify!');
  const macroregCell = eternal('shen.*macroreg*');
  const macrosCell = eternal('*macros*');
  const evalCell = eternal('eval');
  const propertyVectorCell = eternal('*property-vector*');
  ...
  const declareType = await (name, type) => {
    await setSignedFuncs.f(symbolOf(name), type);
    await define('shen.type-signature-of-' + name, async (goal, proc, cont) => {
      // TODO: collect free type variables
      // TODO: create pvar for each type variable
      // TODO: substitute pvars into type
      // TODO: return await unify.f(goal, varType, proc, cont);
    });
  };
  const defineTyped = (name, type, f) => {
    if (async) {
      return (async () => {
        ...
        // TODO: declareType(name, await curryType(toListTree(type)));
        await declare(id, await curryType(toListTree(type)));
        return id;
      })();
    } else {
      const id = define(name, f);
      // TODO: declareType(name, curryType(toListTree(type)));
      declare(id, curryType(toListTree(type)));
      return id;
    }
  };
  ...
  const evalShen = expr => run(evalCell.f(toListTree(expr)));
  ...
  await define('shen-script.lookup-function', x => eternal(nameOf(asSymbol(x))).f || null);
  ...
  return Object.assign($, {
    caller, define, defineTyped, defmacro, evalShen, exec, execEach, inline, load, parse, symbol
  });
};
