...
const {
  Async, Await, Binary, Block, Call, Class, Debugger, Generator, Id, Literal, Member, New,
  NewTarget, Procedure, Return, Slot, Spread, Static, Super, This, Unary, Yield, YieldMany,
  ann
} = require('./ast');
...
module.exports = $ => {
  ...
  const $ConsToArray = args => Call(Member(Id('$'), Id('consToArray')), args);
  ...
  inline('js.this',                    () => This);
  inline('js.super',                 args => Super([Spread($ConsToArray(args))]));
  inline('js.block',                 body => Block($ConsToArray(body)));
...
//  inline('js.new',   (clazz, args) => New(clazz, [Spread($ConsToArray(args))])); // TODO: this isn't building the args array correctly
  ...
  inline('js.bit-not',           x => ann('Number', Unary('~',    x)));
  inline('js.bit-and',      (x, y) => ann('Number', Binary('&',   x, y)));
  inline('js.bit-or',       (x, y) => ann('Number', Binary('|',   x, y)));
  inline('js.bit-xor',      (x, y) => ann('Number', Binary('^',   x, y)));
  inline('js.shift-left',   (x, y) => ann('Number', Binary('<<',  x, y)));
  inline('js.shift-right',  (x, y) => ann('Number', Binary('>>',  x, y)));
  inline('js.shift-right0', (x, y) => ann('Number', Binary('>>>', x, y)));
  inline('js.defined?',          x => ann('JsBool', Binary('===', Unary('typeof', x), Literal('undefined'))));
