...
module.exports = $ => {
  const {
    asCons, asSymbol, async, cons, consFromArray, consToArray, f, fun, future, nameOf,
  ...
  define('js.class', (className, baseClassName, slots) => {});
    ...
    if (Array.isArray(expr) && expr[0] === symbolOf('{') && expr[expr.length - 1] === symbolOf('}')) {
      return [
        ...
        expr
          ...
          .reduceRight((tail, head) => [symbolOf('cons'), head, tail], [])
      ];
    }
  });
