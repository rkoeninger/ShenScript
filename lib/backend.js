...
const idle = symbol => {
  ...
  if (name[0] === name[0].toUpperCase()) {
    ...
    return fabricate(placeholder, Assoc.empty.assoc(placeholder, Template(Member$('s'), name)));
  }
};
...
const buildApp = (context, [fExpr, ...argExprs]) =>
  compose(
    ...
    isSymbol(fExpr)    ? fabricate(Member$f(nameOf(fExpr))) : // TODO: hoist eternal cell (optimization)
...
const Cell = class {
  constructor(s) {
    ...
    this.s = s; // interned symbol itself
  }
};
...
module.exports = (options = {}) => {
  ...
  const eternal = id => {
    let value = globals.get(id);
    if (!value) {
      value = new Cell(symbolOf(id));
  ...
  const atomicTypes = ['Number', 'String', 'Symbol', 'Stream', 'Null']; // TODO: use Set
  const inlines = {
    '=': (x, y) =>
