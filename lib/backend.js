...
const {
  Assoc, AsyncFunction,
  butLast, last, produce, produceState, raise, s // TODO: rename butLast to most
} = require('./utils');
...
const Assoc = class {
  ...
  has(key)    { return lookup(key) !== undefined; }
  get(key) {
...
const Fabr = class {
  ...
  get anySubs()   { return this.subs.size > 0; }
...
// https://github.com/rkoeninger/ShenScript/blob/c300aa012f68bb7647be5ec0e6a1e560b9be9add/lib/backend.js
...
const ann = (dataType, ast) => ast instanceof Fabr ? ann(dataType, ast.ast) : Object.assign(ast, { dataType });
const cast = (dataType, ast) => ast instanceof Fabr ? cast(dataType, ast.ast) : dataType !== ast.dataType ? Object.assign(Call$('as' + dataType, [ast]), { dataType }) : ast;
const uncast = ast => ast instanceof Fabr ? uncast(ast.ast) : ast.dataType === 'JsBool' ? cast('ShenBool', ast) : ast;
...
const buildDo = (context, [_do, ...exprs]) =>
  compose(
    Sequence,
    ...butLast(exprs).map(x => build(context.now(), x)),
...
const buildDefun = (context, [_defun, symbol, params, body]) =>
  compose(
    b => Call$('d', [inlineName(context, symbol), b]),
...
const buildSet = (context, [_set, symbol, value]) =>
  compose(
    x => Assign(Member(Member$('symbols'), inlineName(context, symbol)), x),
    uncast(build(context.now(), value)));
const buildValue = (context, [_value, symbol]) =>
  fabricate(Call$('valueOf', [inlineName(context, symbol)])); // FIXME: inlineName() doesn't fabricate
...
const buildPre = (context, [fExpr, ...argExprs]) =>
  context.preprocessors[nameOf(fExpr)](...argExprs);
