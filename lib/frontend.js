...
module.exports = $ => {
  ...
  /** Returns parsed Shen source code as a cons tree. */
  const parse = source => run(functions['read-from-string'](source));

  /** Returns a function that invokes the function by the given name, settling returned Trampolines. */
  const caller = name => (...args) => run(functions[name](...args));

  /** Defines Shen function that defers to given JavaScript function. */
  const define = (name, f) => {
  ...
  /**
   * Defines Shen function that defers to given JavaScript function and declares with the specified Shen type
   * signature. Type signature has the same structure as in Shen source code, but in array tree form.
   */
  ...
  /** Defines a Shen macro in terms of the given JavaScript function. */
  ...
  /** Evaluates Shen expression tree in isolated environment. */
  const evalShen = expr => run(functions.eval(valueFromArrayTree(expr)));

  /** Parses string as Shen source, evaluates each expression and returns an array of the results. */
  ...
  /** Parses string as Shen source, evaluates each expression and returns last result. */
  const exec = async
    ? async source => last(await execEach(source))
    :       source => last(execEach(source));

  /** Declares a global symbol with the given value and a function by the same name that retrieves the value. */
  ...
  /** Loads Shen code from the given file path. */
  ...
  return Object.assign($, {
    caller, define, defineTyped, defmacro, evalShen, exec, execEach, load, symbol, inline, parse, pre
  });
};
