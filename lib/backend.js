...
const funSync = (f, arity) =>
  (...args) =>
    ...
    Object.assign(funSync((...more) => f(...args, ...more), arity - args.length), { arity: f.arity - args.length, fund: true });
const funAsync = (f, arity) =>
  async (...args) =>
    ...
    Object.assign(funAsync((...more) => f(...args, ...more), arity - args.length), { arity: f.arity - args.length, fund: true });
...
module.exports = (options = {}) => {
  ...
  const defun = (name, f) => (lookup(name).f = f.fund ? f : fun(f), symbolOf(name));
  const $ = {
    cons, toArray, toArrayTree, toList, toListTree,
    asJsBool, asShenBool, asNzNumber, asNeString, inlines, globals, lookup,
    ...
    async: !!options.async, b: bounce, d: defun, l: fun, r: toList, s, t: settle, u: future, c: lookup
  };
