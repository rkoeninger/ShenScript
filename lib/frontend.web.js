...
module.exports = async $ => {
  ...
  const createEventHandler = (name, f) => Object.assign(f, { eventName: name });
  const createElement = (name, args) => args.map(buildDomTree).reduce((element, child) => (
      child instanceof Attr       ? element.setAttribute(child.name, child.value) :
      typeof child === 'function' ? element['on' + child.eventName] = e => (run(child(e)), null) :
  ...
  const buildDomTree = tree =>
    ...
    tree[0] === s`$` ? (tree.length !== 2 ? raiseInvalidForm(tree) : document.createTextNode(tree[1])) :
    tree[0] === s`@` ? (tree.length !== 3 ? raiseInvalidForm(tree) : createAttribute(tree[1], tree[2])) :
    tree[0] === s`!` ? (tree.length !== 3 ? raiseInvalidForm(tree) : createEventHandler(tree[1], tree[2])) :
