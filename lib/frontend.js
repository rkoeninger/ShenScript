...
const { AsyncFunction, GeneratorFunction, last, mapAsync, pairs } = require('./utils');
const {
...
const frontend = $ => {
  const {
    asCons, asNumber, asJsBool, asList, asShenBool, asSymbol, async, compile, cons, consToArray, evalJs,
    fun, functions, future, inlines, isArray, nameOf, preprocessors, s, settle, symbolOf, symbols, valueOf,
    valueFromArrayTree, valueToArrayTree
  } = $;
  ...
  define('js.ast.object',            props => NewObj(pairs(valueToArrayTree(props)).map(([x, y]) => Property(x, y))));
  ...
  define('js.ast.function',  (i, ps, body) => Procedure(i, consToArray(ps), body));
  define('js.ast.function*', (i, ps, body) => Generator(i, consToArray(ps), body));
  ...
  define('js.parseInt',           x => parseInt(x, 10));
  ...
  define('js.obj', values =>
    pairs(consToArray(asList(values)))
