...
const buildApp = (context, [fExpr, ...argExprs]) =>
  complete(context,
    context.has(fExpr) || context.sysFuncs.includes(fExpr) ? SafeId(nameOf(fExpr)) :
...
module.exports = (options = {}) => {
  ...
  const inlines = {
    ...
    // TODO: removing these will solve errors not printed on repl bug?
  ...
  const context = new Context({
    async: options.async, head: true, locals: new Map(), inlines, preprocessors, sysFuncs: options.sysFuncs || []
  });
  const compile = expr => uncast(build(context, expr));
  const compileLambda = (params, body) => uncast(buildLambda(context, params, body));
  ...
  const $ = {
    ...
    symbolOf, nameOf, valueOf, show, equate, raise, fun, bounce, settle, future, compile, compileLambda, defun, trapSync, trapAsync,
