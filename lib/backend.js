...
module.exports = (options = {}) => {
  ...
  const $ = {
    ...
    async: options.async, b: bounce, d: defun, f: functions, r: consFromArray, s, t: settle, u: future
  };
  const F = context.async ? AsyncFunction : Function;
  const evalJs = str => F('$', `return ${str};`)($);
  const evalKl = expr => F('$', generate(Return(compile(valueToArrayTree(expr)))))($);
