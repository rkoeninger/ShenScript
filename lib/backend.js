const {
  Arrow, Assign, Binary, Block, Call, Catch, Conditional, Id, Literal,
  ...
  Call(
    Arrow([], Block(
      Try(
        ...
        Catch(
          isForm(expr[2], 'lambda', 3) ? SafeId(nameOf(expr[2][1])) : Id('e$'),
          Block(Return(
            isForm(expr[2], 'lambda', 3)
              ? uncast(build(context.add([asSymbol(expr[2][1]), { dataType: 'Error' }]), expr[2][2]))
              : Call(uncast(build(context, expr[2])), [Id('e$')])))))),
      context.async),
    [],
    context.async);
