...
module.exports = $ => {
  ...
  const curryType = caller('curry-type');
  ...
  const define = (name, f) => {
    ...
    if (async) {
      return (async () => {
        await put(nameSymbol, s`shen.lambda-form`, funf, valueOf('*property-vector*'));
        await put(nameSymbol, s`arity`, f.length, valueOf('*property-vector*'));
        return nameSymbol;
      })();
    } else {
      put(nameSymbol, s`shen.lambda-form`, funf, valueOf('*property-vector*'));
      put(nameSymbol, s`arity`, f.length, valueOf('*property-vector*'));
      return nameSymbol;
    }
  };
  const defineTyped = (name, type, f) => {
    if (async) {
      return (async () => {
        ...
        await declare(nameSymbol, toListTree(type));
        return nameSymbol;
      })();
    } else {
      const nameSymbol = define(name, f);
      declare(nameSymbol, curryType(toListTree(type)));
      return nameSymbol;
    }
  };
  ...
  const funType = (first, ...rest) =>
    rest.length === 0
      ? [s`-->`, symbolOf(first)]
      : [symbolOf(first), ...flatMap(rest, x => [s`-->`, symbolOf(x)])]; // TODO: this doesn't curry as intended
  ...
  symbol('parallel.cluster-count', 4);

  // TODO: control number of clusters with parallel.*cluster-count* ?
  //       right now, for parallel.map and .filter, every element gets it's own Promise
  //       at the same time, this could start 20000 http requests concurrently
  //       might want to be able to control number of Promises created at a given time

  // TODO: need to create N workers to do parallel processing work

  // TODO: these functions should really be called web.fetch* or something
  //       as creating multiple deferred callbacks is the only utility they have
  ...
  defineTyped(
    ...
    (f, xs) =>
      Promise.all(toArray(xs).map(x => run(f(x))))
        .then(a => toList(a)));
  defineTyped(
    'parallel.filter',
    [[s`A`, s`-->`, s`boolean`], s`-->`, [s`list`, s`A`], s`-->`, [s`list`, s`A`]],
    (f, xs) =>
      Promise.all(toArray(xs).map(async x => ({ pass: asJsBool(await run(f(x))), value: x })))
        .then(a => toList(a.filter(x => x.pass).map(x => x.value))));
  defineTyped(
    'parallel.reduce',
    [[s`A`, s`-->`, s`A`, s`-->`, s`A`], s`-->`, [s`list`, s`A`], s`-->`, s`A`],
    (f, xs) => {
      const array = toArray(xs);
      const clusters = [[]]; // TODO: control number of clusters with parallel.*cluster-count* ?
      // parallel.reduce doesn't make sense without clusters
      return Promise.all(clusters.map(cluster => null))
        .then(results => null);
    });
