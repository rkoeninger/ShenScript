...
module.exports = $ => {
  const {
    ...
    eternal, evalJs, fun, future, inlines, isArray, nameOf, preprocessors, s, settle,
    symbolOf, toArray, toArrayTree, toListTree, valueOf
  } = $;
  const run = async ? future : settle;
  const caller = name => {
    const cell = eternal(name);
  ...
  const define = (name, f) => {
    ...
    const funf = eternal(name).f = fun(f);
  ...
  const defmacro = (name, f) => {
    ...
    if (!toArray(macroreg).includes(nameSymbol)) {
      eternal('shen.*macroreg*').set(cons(nameSymbol, macroreg));
      eternal('*macros*').set(cons(fun(x => macrofn(x)), macros));
    }
    return nameSymbol;
  };
  const eternalEval = eternal('eval');
  const evalShen = expr => run(eternalEval.f(toListTree(expr)));
  ...
  const symbol = (name, value) => {
    ...
    return eternal(starredName).set(value);
  };
  ...
  define('js.ast.for-await-of', (x, xs, body) => ForOf(x, xs, body, true));
  ...
  define('js.ast.compile', x => compile(toArrayTree(x)));
  ...
  defineTyped('js.>>',                   number_number_number, (x, y) => asNumber(x) >> asNumber(y));
  defineTyped('js.>>>',                  number_number_number, (x, y) => asNumber(x) >>> asNumber(y));
  defineTyped('js.clear',                timeout_unit,              x => (clearTimeout(x), null));
  ...
  defineTyped('js.delay',                number_lazy_timeout,  (t, f) => delay(asNumber(t), asFunction(f)));
  ...
  defineTyped('js.repeat',               number_lazy_timeout,  (t, f) => repeat(asNumber(t), asFunction(f)));
  defineTyped('js.sleep',                number_unit,               t => sleep(asNumber(t)));
  ...
  define('shen-script.lookup-function', x => eternal(nameOf(asSymbol(x))).f || null);
