...
module.exports = $ => {
  ...
  const pre = (name, fn) => preprocessors[name] = fn;
  ...
  // TODO: still doesn't do what i wanted
  //       there needs to be a pre-process inline
  //       inline() is after child exprs have been compiled
  ...
  define('js.unchecked.%',                            (x, y) => x % y);
  define('js.unchecked.**',                           (x, y) => x ** y);
  define('js.unchecked.bitwise.not',                  (x, y) => ~x);
  ...
  return Object.assign($, {
    caller, define, defineTyped, defmacro, evalShen, exec, execEach, load, symbol, inline, parse
  });
};
