...
// TODO: might need to bring this back for better type inference
const Assoc = class {
  constructor(entries = []) {
...
// TODO: make this ([JsAst] -> (JsAst | Fabr)) -> [Fabr] -> Fabr
const compose = (f, ...fabrs) => fabricate(f(...fabrs.map(x => x.ast)), Object.assign({}, ...fabrs.map(x => x.subs)));
...
const buildGlobal = (context, name) => {
  ...
  return fabricate(Member(placeholder, Id('f')), { [placeholder.name]: Call(Member$('e'), [Literal(name)]) });
};
...
const hoist = (context, fabr) => {
  const entries = Object.entries(fabr.subs);
  const keys = entries.map(([k]) => k);
  const values = entries.map(([_, v]) => v);
  return fabr.subs.size === 0
    ? fabr
    : fabricate(Call(Arrow(keys.map(x => Id(x)), fabr.ast, context.async), values, context.async));
};
