...
const Assoc = class {
  ...
  get keys()   { return this.entries.map(([k]) => k); }
  get values() { return this.entries.map(([_, v]) => v); }
  lookup(key)  { return this.entries.find(([k]) => equate(k, key)); }
  copy()       { return new Assoc(this.entries.map(([k, v]) => [k, v])); }
  has(key)     { return this.lookup(key) !== undefined; }
  get(key) {
  ...
  assoc(key, value) {
    ...
    const entry = copy.lookup(key);
    if (entry) {
      entry.value = value;
    } else {
      copy.entries.push([key, value]);
    }
    return copy;
  }
  static reduce(xss) {
    ...
    const ys = xss[0].copy();
    for (const xs of xss.slice(1)) {
      for (const [key, value] of xs.entries) {
        const entry = ys.lookup(key);
        if (entry) {
          // TODO: instead of overwriting, just assert that values match
          //       if they don't, there will be errors later
          entry.value = value;
        } else {
          ys.entries.push([key, value]);
        }
      }
    }
    return ys;
...
Assoc.empty = new Assoc();
Assoc.single = (k, v) => new Assoc([[k, v]]);

const Fabr = class {
  constructor(ast, subs = Assoc.empty) {
    this.ast = ast;
    this.subs = subs;
  }
  sub(key, value) { return new Fabr(this.ast, this.subs.assoc(key, value)); }
};
const fabricate = (ast, subs) => new Fabr(ast, subs);
const compose = (f, ...fabrs) => fabricate(f(...fabrs.map(x => x.ast)), Assoc.reduce(fabrs.map(x => x.subs)));
...
// TODO: Member$e
// TODO: need and inject function for building references to eternals
const Member$ = name => Member(Id('$'), Id(name));
const Member$f = name => Member(Call(Member$('e'), [Literal(name)]), Id('f'));
...
const ann = (dataType, ast) => ast instanceof Fabr ? compose(x => ann(dataType, x), ast) : Object.assign(ast, { dataType });
const cast = (dataType, ast) => ast instanceof Fabr ? compose(x => cast(dataType, x), ast) : dataType !== ast.dataType ? Object.assign(Call$('as' + dataType, [ast]), { dataType }) : ast;
const uncast = ast => ast instanceof Fabr ? compose(x => uncast(x), ast) : ast.dataType === 'JsBool' ? cast('ShenBool', ast) : ast;
...
const idle = symbol => {
  ...
  if (name[0] === name[0].toUpperCase()) {
    ...
    // FIXME: doing this broke idle symbols in some contexts, there were free variables
    //        and functions that weren't getting properly defined
    const placeholder = ann('Symbol', SafeId(name, '$sym$'));
    return fabricate(placeholder, Assoc.single(placeholder, Template(Member$('s'), name)));
  }
};
...
const buildLet = (context, [_let, symbol, binding, body]) => // TODO: build let with ((X=1,Y=2)=>X+Y)() instead of ((X,Y)=>X+Y)(1,2)
  compose(
...
const buildSet = (context, [_set, symbol, value]) =>
  compose(
    (s, x) => Assign(Member(Call(Member$('e'), [s]), Id('v')), x), // TODO: hoist eternal cell (optimization)
    inlineName(context, symbol),
    uncast(build(context.now(), value)));
const buildValue = (context, [_value, symbol]) =>
  compose(
    s => Call$('valueOf', [s]), // TODO: hoist eternal cell (optimization)
    inlineName(context, symbol));
...
const buildPre = (context, [fExpr, ...argExprs]) =>
  fabricate(context.preprocessors[nameOf(fExpr)](...argExprs));
const buildGlobal = (context, name) => {
  const placeholder = SafeId(name, '$e$');
  return fabricate(Member(placeholder, Id('f')), Assoc.single(placeholder, Call(Member$('e'), [Literal(name)])));
};
const buildApp = (context, [fExpr, ...argExprs]) =>
  compose(
    (f, ...args) => complete(context, f, args, context.async),
    context.has(fExpr) ? fabricate(SafeId(nameOf(fExpr))) :
    isArray(fExpr)     ? uncast(build(context.now(), fExpr)) :
    isSymbol(fExpr)    ? fabricate(Member$f(nameOf(fExpr))) :
    //isSymbol(fExpr)    ? buildGlobal(context, nameOf(fExpr)) : // FIXME: causes stack overflow on render
...
const hoist = (context, fabr) =>
  fabr.subs.size === 0
    ? fabr
    : fabricate(Call(Arrow(fabr.subs.keys, fabr.ast, context.async), fabr.subs.values, context.async));

// TODO: Iife that takes params, args
// TODO: isForm(expr, 0, 'cond')

const Cell = class {
  constructor() {
    this.f = undefined; // global function
    this.v = undefined; // global symbol value
  }
};

module.exports = (options = {}) => {
  // TODO: also need to have mutable reference cells that contain global functions,
  //       so those cells can be enclosed over, not just the current value of the function
  const globals = new Map();
  const eternal = id => {
    let cell = globals.get(id);
    if (!cell) {
      cell = new Cell();
      globals.set(id, cell);
    }
  ...
  const inlines = {
    ...
    'str':               x => ann('String', Call$('show',     [x].map(uncast))), // TODO: hoist eternal cells for helper functions?
    ...
    'read-byte':         x => ann('Number', Call$f('read-byte',  [x].map(uncast))), // TODO: hoist eternal cells for Call$f'd functions?
  ...
  const functions = {
    ...
    'set':          (x, y) => eternal(nameOf(asSymbol(x))).v = y,
    'value':             x => valueOf(nameOf(asSymbol(x))),
    'type':         (x, _) => x
  };
  Object.entries(functions).forEach(([name, f]) => eternal(name).f = fun(f));
  Object.entries(symbols).forEach(([name, v]) => eternal(name).v = v);
  const valueOf = x => {
    const value = eternal(x).v;
    return value === undefined
      ? raise(`global "${x}" is not defined`)
      : value;
  };
  ...
  const compile = expr => hoist(context, uncast(build(context, expr))).ast;
  const defun = (name, f) => (eternal(name).f = f.fund ? f : fun(f), symbolOf(name));
  const $ = {
    cons, toArray, toArrayTree, toList, toListTree,
    asJsBool, asShenBool, asNzNumber, asNeString, inlines, preprocessors,
    ...
    symbolOf, nameOf, valueOf, show, equate, raise, fun, bounce, settle, future, compile, defun, globals, eternal,
    async: !!options.async, b: bounce, d: defun, f: functions, l: fun, r: toList, s, t: settle, u: future, e: eternal
  };
