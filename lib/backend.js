/**
 * Contains functionality for building and running the Shen kernel.
 * @module backend
 */

const {
  Arrow, Assign, Await, Binary, Block, Call, Conditional, Id, Literal,
...
/** Classic lisp cons cell with a head and a tail (traditionally called car and cdr). */
const Cons = class {
...
/** Trampolines hold a function to apply and arguments to apply it to. Used to implement tail call elimination. */
...
/** Metadata about a KLambda expression tree that accumulates as it is compiled. */
const Context = class {
  constructor(options) {
...
/** Creates a Cons from a head and tail. */
...
/** Determines if two values are equal according to the semantics of Shen. */
const equate = (x, y) =>
  x === y
...
/** Creates a trampoline from function and arguments. */
const bounce = (f, ...args) => new Trampoline(f, args);

/**
 * If value is a Trampoline, runs Trampoline and repeats. settle(...) calls are
...
/**
 * Awaits argument, if value is a Trampoline, runs Trampoline and repeats. future(...) calls
...
/**
 * Invoke first function, settle result and return it. If an error
...
/**
 * Invoke first function, settle result, await and return it. If an error
 * is raised, apply second function to error and return.
 */
...
const buildLet = (context, [_let, symbol, binding, body]) => {
  ...
  const async = context.async && anyAwaits(bodyAst);
  return Call(Arrow([SafeId(nameOf(symbol))], bodyAst, async), [build(context.now(), binding)], async);
};
const buildLambda = (context, params, body) => {
  const bodyAst = uncast(build(context.later().add(...params.map(x => [asSymbol(x), {}])), body));
  return Call$('l', [Arrow(params.map(x => SafeId(nameOf(x))), bodyAst, context.async && anyAwaits(bodyAst))]);
};
const buildTrap = (context, [_trap, body, handler]) =>
  // TODO: don't use a trapAsync unless necessary
  Call$(context.async ? 'trapAsync' : 'trapSync',
    // TODO: call Arrow directly do we don't have $.l() in there
...
const nonAwaits = new Set([
  // TODO: these should be provided by render.js
  s`shen.percent`,
  s`shen.exclamation`,
  s`shen.space`,
  s`shen.tab`,
...
const buildApp = (context, [fExpr, ...argExprs]) =>
  complete(context,
    ...
    context.async && !nonAwaits.has(fExpr));
...
/** Backend environment creation function. */
module.exports = (options = {}) => {
  ...
  /** toString() function. Returns string representation of any value. */
  const show = x =>
    x === null    ? '[]' :
    isString(x)   ? `"${x}"` :
    isSymbol(x)   ? nameOf(x) :
  ...
  /** Global symbol index. Keys are strings. */
  const symbols = {
    '*language*':       'JavaScript',
  ...
  /** Post-processing inlines. Each accepts constructed JavaScript ASTs and returns another AST. */
  const inlines = {
    '=': (x, y) =>
  ...
  /**
   * Returns the value of the given global symbol. Raises an error if it is not defined.
   */
  ...
  /**
   * Global index of functions by name. Contained functions are
  ...
  const preprocessors = {};
  // TODO: rename inlines to postprocessors?
  const context = new Context({
    async: options.async, head: true, locals: new Map(), inlines, preprocessors
  });
  const compile = expr => uncast(build(context, expr));

  /** Adds function to the global function registry. */
  const defun = (name, f) => (functions[name] = f.fund ? f : fun(f), symbolOf(name));

  /**
   * Shen environment object. Contains global function and symbol registries, compilation and evaluation
   * functions, type recognition and checking functions and some language primitives.
   */
  const $ = {
  ...
  /** Evalutes a JavaScript AST in isolated scope with access to $. */
  const evalJs = ast => F('$', generate(ast.isStmt ? Block(ast) : Return(ast)))($);

  /** Builds and evaluates a KLambda expression tree in isolated scope with access to $. */
