...
const { ann, Binary, Literal, NewTarget, Super, This, Unary } = require('./ast');
...
module.exports = $ => {
  ...
  inline('js.this',             () => This);
  inline('js.super/0',          () => Super([])); // TODO: add a flag for special forms with variable args?
  inline('js.super/1',           x => Super([x]));
  inline('js.super/2',      (x, y) => Super([x, y]));
  inline('js.super/3',   (x, y, z) => Super([x, y, z]));
  inline('js.==',                x => ann('JsBool', Binary('==',  x, y)));
  inline('js.===',               x => ann('JsBool', Binary('===', x, y)));
  inline('js.not',               x => ann('JsBool', Unary('!', x)));
  inline('js.and',          (x, y) => Binary('&&', x, y));
  inline('js.or',           (x, y) => Binary('||', x, y));
  inline('js.bit-not',           x => ann('Number', Unary('~',    cast('Number', x))));
  inline('js.bit-and',      (x, y) => ann('Number', Binary('&',   cast('Number', x), cast('Number', y))));
  inline('js.bit-or',       (x, y) => ann('Number', Binary('|',   cast('Number', x), cast('Number', y))));
  inline('js.bit-xor',      (x, y) => ann('Number', Binary('^',   cast('Number', x), cast('Number', y))));
  inline('js.shift-left',   (x, y) => ann('Number', Binary('<<',  cast('Number', x), cast('Number', y))));
  inline('js.shift-right',  (x, y) => ann('Number', Binary('>>',  cast('Number', x), cast('Number', y))));
  inline('js.shift-right0', (x, y) => ann('Number', Binary('>>>', cast('Number', x), cast('Number', y))));
