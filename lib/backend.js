...
const recognise = (context, expr) => {
  if (isArray(expr) && expr.length === 2 && isSymbol(expr[0]) && isSymbol(expr[1])) {
...
module.exports = (options = {}) => {
  const context = new Context({ async: options.async, head: true, locals: new Map(), inlines: new Map() });
  ...
  $.evalJs = ast => F('$', generate(isStatement(ast) ? Block(ast) : Return(ast)))($);
