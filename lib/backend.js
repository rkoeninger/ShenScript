...
// TODO: use custom Assoc type to track expression types
//       allows keeping track of cons chains, avoid unnecessary asCons()
const Context = class {
  constructor(options) {
...
const canInline = (context, expr) =>
  ...
  (context.inlines[nameOf(expr[0])].length || context.inlines[nameOf(expr[0])].arity) === expr.length - 1;
...
module.exports = (options = {}) => {
  ...
  Object.entries(symbols).forEach(([name, v]) => lookup(name).set(v));
  const functions = {
    ...
    'set':          (x, y) => lookup(nameOf(asSymbol(x))).set(y),
    'value':             x => valueOf(nameOf(asSymbol(x))),
    'type':         (x, _) => x
  };
  Object.entries(functions).forEach(([name, f]) => lookup(name).f = fun(f));
  const inlines = {};
  const annMaybe = (dataType, ast) => dataType ? ann(dataType, ast) : ast;
  const inline = (name, dataType, paramTypes, f) =>
    inlines[name] =
      Object.assign(
        (...args) => annMaybe(dataType, f(...args.map((a, i) => paramTypes[i] ? cast(paramTypes[i], a) : uncast(a)))),
        { arity: f.length });
  const strictlyEqualTypes = new Set(['Number', 'String', 'Symbol', 'Stream', 'Null', 'ShenBool']);
  const comparable = (x, y) => strictlyEqualTypes.has(x.dataType) || strictlyEqualTypes.has(y.dataType);
  inline('=',               'JsBool', [null, null],           (x, y) => comparable(x, y) ? Binary('===', x, y) : Call$('equate', [x, y]));
  ...
  inline('cons',            'Cons',   [null, null],                x => Call$('cons', [x, y]));
  ...
  // TODO: need build() -level inlining for nested tlstr's?
  ...
  const $ = {
    ...
    asJsBool, asShenBool, isEArray, isNeArray, asNeString, asNzNumber, globals, lookup,
    ...
    symbolOf, nameOf, valueOf, show, equate, raise, fun, bounce, settle, assemble, construct, compile, defun, inline,
    async: !!options.async, b: bounce, d: defun, l: fun, r: toList, s, t: settle, c: lookup
  };
