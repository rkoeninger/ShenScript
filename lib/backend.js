...
const nameOfSymbol = (context, symbol) =>
...
const buildLet = (context, [_let, symbol, binding, body]) =>
  build(context, appears(symbol, body) ? [[s`lambda`, symbol, body], binding] : [s`do`, binding, body]);
...
const buildDefun = (context, [_defun, symbol, params, body]) =>
  Call$('d', [nameOfSymbol(context, symbol), buildLambda(context.clear(), params, body)]);
const buildCons = (context, expr) => {
  ...
  return ann('Cons', Call$('consFromArray',
...
const buildSet = (context, [_set, symbol, value]) =>
  Assign(
    Member(Member$('symbols'), nameOfSymbol(context, symbol)),
    uncast(build(context.now(), value)));
const buildValue = (context, [_value, symbol]) =>
  Call$('valueOf', [nameOfSymbol(context, symbol)]);
...
module.exports = (options = {}) => {
  ...
  const $ = {
    ...
    d: defun, f: functions, s, b: bounce, t: settle, u: future, async: options.async
  };
  const Func = context.async ? AsyncFunction : Function;
  const evalJs = str => Func('$', `return ${str};`)($);
  const evalKl = expr => Func('$', generate(Return(compile(valueToArrayTree(expr)))))($);
