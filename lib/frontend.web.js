...
module.exports = async $ => {
  const {
    asShenBool, asString, async, caller, define, defineTyped, defun, evalJs, inline, isFunction, isNeArray,
    isString, isSymbol, lookup, nameOf, raise, run, s, show, symbol, symbolOf, toArray, toArrayTree, toList, valueOf
  } = $ = await frontend($);
  ...
  if (async) {
    ...
    await defun('y-or-n?', m => asShenBool(confirm(m)));
  ...
  const buildElement = (element, children) => (
    children.forEach(child =>
      ...
      isFunction(child)     ? element['on' + child.event] = e => (run(child(e)), null) :
  ...
  await define('dom.onready', f => (onReady(() => run(f())), null));
