...
const ForOf = (left, right, body, await = false) => ({ type: 'ForOfStatement', left, right, body, await });
...
const SafeId = (sym, suffix = '') => Id(avoidReserved(escapeName(Symbol.keyFor(sym)) + suffix));
const Sequence = (...expressions) => ({ type: 'SequenceExpression', expressions });
const Slot = (kind, key, value) => ({ type: 'MethodExpression', key, computed: key.type !== 'Identifier', kind, value });
...
const anyAwaits = ast =>
  ast && (
    ...
    (Array.isArray(ast) && ast.some(anyAwaits)) ||
    typeof ast === 'object' && Object.values(ast).some(anyAwaits));

module.exports = {
  Arrow, Assign, Async, Await, Binary, Block, Call, Catch, Class, Conditional, Const, Debugger, Empty, For,
  ForIn, ForOf, Generator, Id, If, Let, Literal, Local, Member, New, NewObj, NewTarget, Procedure, Program,
  Property, Return, SafeId, Sequence, Slot, Spread, Statement, Static, Super, TaggedTemplate, Template,
