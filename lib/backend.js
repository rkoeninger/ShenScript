...
const hoist = (fabr, async = false) =>
  Object.keys(fabr.subs).length === 0
    ? fabr
    : fabricate(Iife(fabr.keys.map(x => Id(x)), fabr.values, fabr.ast, async));
const inject = (name, f) => {
...
const ann = (dataType, ast) =>
  ast instanceof Fabr ? assemble(x => ann(dataType, x), ast) :
  Object.assign(ast, { dataType });
const cast = (dataType, ast) =>
  ast instanceof Fabr ? assemble(x => cast(dataType, x), ast) :
  dataType !== ast.dataType ? Object.assign(Call$('as' + dataType, [ast]), { dataType }) :
  ast;
const uncast = ast =>
  ast instanceof Fabr ? assemble(x => uncast(x), ast) :
...
module.exports = (options = {}) => {
  ...
  const compile = expr => hoist(construct(expr), context.async).ast;
  const defun = (name, f) => (eternal(name).f = f.fund ? f : fun(f), symbolOf(name));
  const $ = {
    cons, toArray, toArrayTree, toList, toListTree,
    asJsBool, asShenBool, asNzNumber, asNeString, inlines, preprocessors, globals, eternal,
