...
const buildInline = (context, [fExpr, ...argExprs]) =>
  (context.async && context.awaitedInlines.includes(nameOf(fExpr)) ? Await : (x => x))(
...
module.exports = (options = {}) => {
  ...
  const awaitedInlines = ['read-byte', 'write-byte'];
  const inlines = {
    '=': (x, y) =>
  ...
  const context = new Context({
    async: options.async, head: true, locals: new Map(), inlines, preprocessors, awaitedInlines
  });
  ...
  const $ = {
    ...
    asJsBool, asShenBool, asNzNumber, asNeString, symbols, inlines, preprocessors, awaitedInlines, functions,
