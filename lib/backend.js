...
const idle = symbol => {
  ...
  if (name[0] === name[0].toUpperCase()) {
    return fabricate(Template(Member$('s'), nameOf(symbol)));
  } else {
    ...
    const placeholder = ann('Symbol', SafeId(nameOf(symbol), '$sym$'));
    return fabricate(placeholder, Assoc.empty.assoc(placeholder, Template(Member$('s'), nameOf(symbol))));
  }
};
...
const buildSet = (context, [_set, symbol, value]) =>
  compose(
    (s, x) => Assign(Member(Member$('symbols'), s), x), // TODO: hoist eternal cell
    inlineName(context, symbol),
    uncast(build(context.now(), value)));
const buildValue = (context, [_value, symbol]) =>
  compose(
    s => Call$('valueOf', [s]), // TODO: hoist eternal cell
    inlineName(context, symbol));
...
const buildApp = (context, [fExpr, ...argExprs]) =>
  compose(
    ...
    isSymbol(fExpr)    ? fabricate(Member$f(nameOf(fExpr))) : // TODO: hoist eternal cell
...
module.exports = (options = {}) => {
  ...
  const Cell = class {
    constructor() {
      ...
      this.s = undefined; // interned symbol itself
    }
  };
  const globals = {}; // TODO: use Map instead of object?
  const eternal = /*TODO: symbol or string?*/ id => globals[id] || (globals[id] = new Cell());
  // TODO: $.e as an alias for $.eternal
  ...
  // TODO: use globals<id, Cell> instead of symbols, functions
  ...
  const defun = (name, f) => (functions[name] = f.fund ? f : fun(f), symbolOf(name)); // TODO: Update to lookup Cell
  const $ = {
