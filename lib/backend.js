...
const idle = symbol =>
  // FIXME: taking a shot at hoisting idle symbol
  // FIXME: should only do this in certain situations i guess, maybe use context to see if we're in one of those
  ann('Symbol', fabricate(SafeId(nameOf(symbol), '$sym$'), Assoc.empty.assoc(SafeId(nameOf(symbol), '$sym$'), Template(Member$('s'), nameOf(symbol)))));
...
const build = (context, expr) =>
  ...
  isSymbol(expr) ? fabricate(context.has(expr) ? variable(context, expr) : idle(expr)) :
...
const hoist = (context, fabr) =>
  fabr.subs.size === 0
    ? fabr
    : fabricate(Call(
        Arrow(fabr.subs.keys, fabr.ast, context.async), // TODO: which one is the key and which is the value?
        fabr.subs.values,
        context.asyc));
